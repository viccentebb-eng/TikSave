(() => {
  ...
  function ensureViewer() {
    ...
    root.innerHTML = `
      ...
      ">
        ...
        <div style="display:flex;align-items:center;gap:5px;flex:none">
          ...
          <button data-ts-download title="Descargar" style="${buttonStyle("auto")}">Descargar</button>
    ...
    root.querySelector("[data-ts-tab]").addEventListener("click", () => {
      if (currentUrl) browser.runtime.sendMessage({ type: "openImageInTab", url: currentUrl }).catch(() => {});
    });
    root.querySelector("[data-ts-download]").addEventListener("click", () => {
      if (currentUrl) browser.runtime.sendMessage({ type: "downloadDirectImage", url: currentUrl }).catch(() => {});
    });
